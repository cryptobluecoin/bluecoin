...
TMPDIR_PREFIX = "bluecoin_func_test_"
...
class BluecoinTestMetaClass(type):
    """Metaclass for BluecoinTestFramework.

    Ensures that any attempt to register a subclass of `BluecoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'BluecoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("BluecoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("BluecoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class BluecoinTestFramework(metaclass=BluecoinTestMetaClass):
    ...
    def setup(self):
        ...
        fname_bluecoind = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "bluecoind" + config["environment"]["EXEEXT"],
        )
        fname_bluecoincli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "bluecoin-cli" + config["environment"]["EXEEXT"],
        )
        fname_bluecoinutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "bluecoin-util" + config["environment"]["EXEEXT"],
        )
        self.options.bluecoind = os.getenv("BLUECOIND", default=fname_bluecoind)
        self.options.bluecoincli = os.getenv("BLUECOINCLI", default=fname_bluecoincli)
        self.options.bluecoinutil = os.getenv("BLUECOINUTIL", default=fname_bluecoinutil)
    ...
    def shutdown(self):
        ...
        rpc_logger = logging.getLogger("BluecoinRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'bluecoind', self.options.bluecoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'bluecoin-cli', self.options.bluecoincli) for v in versions]
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as bluecoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    bluecoind=self.options.bluecoind,
                    bluecoin_cli=self.options.bluecoincli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in bluecoin.conf
    ...
    def skip_if_no_bluecoind_tracepoints(self):
        """Skip the running test if bluecoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("bluecoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_bluecoind_zmq(self):
        """Skip the running test if bluecoind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("bluecoind has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if bluecoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("bluecoin-wallet has not been compiled")

    def skip_if_no_bluecoin_util(self):
        """Skip the running test if bluecoin-util has not been compiled."""
        if not self.is_bluecoin_util_compiled():
            raise SkipTest("bluecoin-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if bluecoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("bluecoin-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether bluecoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether bluecoin-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_bluecoin_util_compiled(self):
        """Checks whether bluecoin-util was compiled."""
        return self.config["components"].getboolean("ENABLE_BLUECOIN_UTIL")
