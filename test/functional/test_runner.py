...
`test/functional/test_framework/test_framework.py:BluecoinTestFramework.main`.
...
BASE_SCRIPTS = [
    ...
    'interface_bluecoin_cli.py --legacy-wallet',
    'interface_bluecoin_cli.py --descriptors',
...
def main():
    ...
    enable_bluecoind = config["components"].getboolean("ENABLE_BLUECOIND")

    if not enable_bluecoind:
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control):
    ...
    # Warn if bluecoind is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "bluecoind"], stdout=subprocess.DEVNULL).returncode == 0:
