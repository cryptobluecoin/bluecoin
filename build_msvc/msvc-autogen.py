...
def parse_config_into_blu_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Bluecoin Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/bluecoin_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/bluecoin_config.h'), "w", encoding="utf8") as blu_config:
        blu_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='Bluecoin-core msbuild configuration initialiser.')
    ...
    parse_config_into_blu_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/bluecoin_config.h'), os.path.join(SOURCE_DIR, 'config/bluecoin-config.h'))
