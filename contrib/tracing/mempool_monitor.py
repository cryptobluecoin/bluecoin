...
""" Example logging Bluecoin Core mempool events using the mempool:added,
...
def main(bluecoind_path):
    bluecoind_with_usdts = USDT(path=str(bluecoind_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    bluecoind_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    bluecoind_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    bluecoind_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    bluecoind_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[bluecoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/bluecoind")
        exit(1)
