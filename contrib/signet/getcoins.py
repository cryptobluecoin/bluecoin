...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing bluecoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='bluecoin-cli', help='bluecoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Bluecoin address to which the faucet should send')
...
parser.add_argument('bluecoin_cli_args', nargs='*', help='Arguments to pass on to bluecoin-cli (default: -signet)')

args = parser.parse_args()

if args.bluecoin_cli_args == []:
    args.bluecoin_cli_args = ['-signet']


def bluecoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.bluecoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = bluecoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = bluecoin_cli(['getnewaddress', 'faucet', 'bech32'])
