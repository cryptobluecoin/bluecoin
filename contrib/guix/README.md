# Bootstrappable Bluecoin Core Builds

This directory contains the files necessary to perform bootstrappable Bluecoin
Core builds.
...
To build Bluecoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Bluecoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/bluecoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/bluecoin-core/guix.sigs).
