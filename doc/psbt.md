# PSBT Howto for Bluecoin Core

Since Bluecoin Core 0.17, an RPC interface exists for Partially Signed Bluecoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/bluecoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Bluecoin transactions that are not fully signed
...
#### Multisig with multiple Bluecoin Core instances
...
Bluecoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `bluecoin-cli` is used.
...
Later, when *V* BLU has been received on *Amulti*, and Bob and Carol want to
