Bluecoin Core version 0.11.0 is now available from:

  <https://bluecoin.org/bin/bluecoin-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer BLU/kB fee to be
...
For example, add the following to `bluecoin.conf`:
...
related to the blockchain in the bluecoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Bluecoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Bluecoin Core will 
...
For an example script see [bluecoin-submittx](https://github.com/laanwj/bluecoin-submittx).
...
- #5334 `cf87536` libbluecoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new bluecoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only BluecoinAmountField
- #5753 `0f44672` Add bluecoin logo to about screen
...
- #5809 `46bfbe7` Add bluecoin-cli man page
...
- #5849 `41259ca` contrib/init/bluecoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Bluecoin-Qt.app to Bluecoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for bluecoin-tx)
