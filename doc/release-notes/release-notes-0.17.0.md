Bluecoin Core version 0.17.0 is now available from:

  <https://bitcoincore.org/bin/bluecoin-core-0.17.0/>
...
[BIP 174 PSBT](https://github.com/bluecoin/bips/blob/master/bip-0174.mediawiki) is an interchange format for Bluecoin transactions that are not fully signed
yet, together with relevant metadata to help entities work towards signing it.
It is intended to simplify workflows where multiple parties need to cooperate to
produce a transaction. Examples include hardware wallets, multisig setups, and
[CoinJoin](https://bluecointalk.org/?topic=279249) transactions.
...
- New `fees` field introduced in `getrawmempool`, `getmempoolancestors`, `getmempooldescendants` and
   ...
   and `descendant` denominated in BLU. This new field deprecates previous fee fields, such as
...
- #11962 `0a01843` add seed.bluecoin.sprovoost.nl to DNS seeds (Sjors)
...
- #12240 `6f8b345` Introduced a new `fees` structure that aggregates all sub-field fee types denominated in BLU (mryandao)
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI bluecoin:// (krab)
...
- #13528 `b877c39` Move BluecoinGUI initializers to class, fix initializer order warning (laanwj)
...
- #13486 `66e1a08` Move rpc/util.cpp from libbluecoin-util to libbluecoin-server (ken2812221)
...
- #13604 `75bea05` Add depends 32-bit arm support for bluecoin-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling bluecoin-qt (TheCharlatan)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use BluecoinTestFramework (jnewbery)
...
- #12443 `07090c5` Move common args to bluecoin.conf (MarcoFalke)
...
- #11772 `0630974` Change invalidblockrequest to use BluecoinTestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use BluecoinTestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use BluecoinTestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for BluecoinTestFramework (WillAyd)
...
- #12882 `d96bdd7` Make `test_bluecoin` pass under ThreadSanitzer (clang). Fix lock-order-inversion (potential deadlock) (practicalswift)
...
- #13687 `9d26b69` travis: Check that ~/.bluecoin is never created (MarcoFalke)
...
- #13811 `660abc1` travis: Run `bench_bluecoin` once (MarcoFalke)
...
- #12102 `7fb8fb4` Apply hardening measures in bluecoind systemd service file (Flowdalic)
- #12652 `55f490a` bluecoin-cli: Provide a better error message when bluecoind is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to bluecoind in debian package (ghost)
...
- #13374 `56f6936` utils and libraries: checking for bluecoin address in translations (kaplanmaxe)
...
- #13603 `453ae5e` bluecoin-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix bluecoin-cli --version (Empact)
...
- #12877 `2b54155` Use bluecoind in Tor documentation (knoxcard)
...
- #13346 `60f0358` update bluecoin-dot-org links in release-process.md (fanquake)
...
- #13742 `d2186b3` Adjust bluecoincore.org links (MarcoFalke)
...
- #13905 `73a09b4` fixed bluecoin-cli -help output for help2man (hebasto)
