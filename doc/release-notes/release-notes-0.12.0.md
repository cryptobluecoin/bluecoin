Bluecoin Core version 0.12.0 is now available from:

  <https://bluecoin.org/bin/bluecoin-core-0.12.0/>
...
pre-0.12 versions of Bluecoin Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start Bluecoin Core version 0.11 or
earlier.
...
[libsecp256k1](https://github.com/bluecoin-core/secp256k1) instead of OpenSSL.
...
(https://github.com/bluecoin/bips/blob/master/bip-0130.mediawiki)
...
Bluecoin Core 0.12 will have a strict maximum size on the mempool. The
...
Bluecoin Core 0.12 also introduces new default policy limits on the length and
...
Bluecoin Core 0.12 nodes. Bluecoin Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/bluecoin/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in Bluecoin Core 0.12 does not yet have support for
...
In Bluecoin Core 0.12, when mempool limit has been reached a higher minimum
...
notes of v0.11.0](https://github.com/bluecoin/bluecoin/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/bluecoin/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/bluecoin/bips/blob/master/bip-0065.mediawiki)
...
Then, to tunnel a SSL connection on 219633 to a RPC server bound on localhost on port 29633 do:

    stunnel -d 219633 -r 127.0.0.1:29633 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /bluecoinrpc>
        ProxyPass http://127.0.0.1:19633/
        ProxyPassReverse http://127.0.0.1:19633/
...
- #6214 `87406aa` [OSX] revert renaming of Bluecoin-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new bluecoin-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include bluecoin-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix bluecoin-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of bluecoin-cli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override BLUECOIND and BLUECOINCLI if they're set (Wladimir J. van der Laan)
...
- #6387 `11576a5` [bluecoin-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add BLUECOIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libbluecoinconsensus (Braydon Fuller)
...
- #6504 `b6fee6b` Rationalize currency unit to "BLU" (Ross Nicoll)
