# CJDNS support in Bluecoin Core

It is possible to run Bluecoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Bluecoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Bluecoin network and individual nodes.
...
before it will work with your Bluecoin Core node. This is described in steps
...
## Run Bluecoin Core with CJDNS

Once you are connected to the CJDNS network, the following Bluecoin Core
...
When enabled, this option tells Bluecoin Core that it is running in an
...
IPv6 local network. This helps Bluecoin Core perform better address management:
...
CJDNS support was added to Bluecoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `bluecoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/bluecoin/bluecoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Bluecoin Core

There are several ways to see your CJDNS address in Bluecoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `bluecoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `bluecoin-cli getpeerinfo`).
