# User-space, Statically Defined Tracing (USDT) for Bluecoin Core

Bluecoin Core includes statically defined tracepoints to allow for more
...
**Note**: Bluecoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Bluecoin Core
...
Multiple tools can list the available tracepoints in a `bluecoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Bluecoin Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/bluecoind
stap net        outbound_message 0x0000000000107c05 /src/bluecoind
stap validation block_connected  0x00000000002fb10c /src/bluecoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Bluecoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/bluecoind | grep NT_STAPSDT -A 4 -B 2
