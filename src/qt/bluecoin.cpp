...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<BluecoinUnit>("BluecoinUnit");
#else
    qRegisterMetaType<BluecoinUnit>("BluecoinUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bluecoin_de.qm (shortcut "de" needs to be defined in bluecoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. bluecoin_de_DE.qm (shortcut "de_DE" needs to be defined in bluecoin.qrc)
...
static const char* qt_argv = "bluecoin-qt";

BluecoinApplication::BluecoinApplication()
...
void BluecoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the BluecoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", BluecoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void BluecoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool BluecoinApplication::createOptionsModel(bool resetSettings)
{
...
void BluecoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new BluecoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &BluecoinGUI::quitRequested, this, &BluecoinApplication::requestShutdown);
...
void BluecoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void BluecoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool BluecoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void BluecoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &BluecoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &BluecoinApplication::handleRunawayException);
    connect(this, &BluecoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &BluecoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void BluecoinApplication::parameterSetup()
{
...
void BluecoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void BluecoinApplication::requestInitialize()
{
...
void BluecoinApplication::requestShutdown()
{
...
void BluecoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &BluecoinGUI::handlePaymentRequest);
            connect(window, &BluecoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void BluecoinApplication::handleRunawayException(const QString &message)
{
...
void BluecoinApplication::handleNonFatalException(const QString& message)
{
...
WId BluecoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool BluecoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", BluecoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    /// 6-7. Parse bluecoin.conf, determine network, switch to network specific
