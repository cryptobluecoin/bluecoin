...
/** Call node.updateRwSetting() with Bluecoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Bluecoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/bluecoin/bluecoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because bluecoin 22.x will not try to read these.
...
//! Convert enabled/size values to bluecoin -prune setting.
...
//! Get pruning enabled value to show in GUI from bluecoin -prune setting.
...
//! Get pruning size value to show in GUI from bluecoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayBluecoinUnit")) {
        settings.setValue("DisplayBluecoinUnit", QVariant::fromValue(BluecoinUnit::BLU));
    }
    QVariant unit = settings.value("DisplayBluecoinUnit");
    if (unit.canConvert<BluecoinUnit>()) {
        m_display_bluecoin_unit = unit.value<BluecoinUnit>();
    } else {
        m_display_bluecoin_unit = BluecoinUnit::BLU;
        settings.setValue("DisplayBluecoinUnit", QVariant::fromValue(m_display_bluecoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if bluecoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_bluecoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<BluecoinUnit>() == m_display_bluecoin_unit) return;
    m_display_bluecoin_unit = new_unit.value<BluecoinUnit>();
    QSettings settings;
    settings.setValue("DisplayBluecoinUnit", QVariant::fromValue(m_display_bluecoin_unit));
    Q_EMIT displayUnitChanged(m_display_bluecoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/bluecoin/bluecoin/pull/8273
    ...
    // (https://github.com/bluecoin-core/gui/issues/567).
