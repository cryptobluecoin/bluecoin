This directory contains the source code for the Bluecoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- ClientModel is used by the main application `bluecoingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a bluecoin:... link from a browser).
...
* UI elements like BluecoinAmountField, which inherit from QWidget.
* `bluecoinstrings.cpp`: automatically generated
* `bluecoinunits.(h/cpp)`: BLU / mBLU / etc. handling
* `callback.h`
...
**Note:** Do not change `local/bluecoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `bluecoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `bluecoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `bluecoin-qt`
9. You're all set! Start developing, building, and debugging the Bluecoin Core GUI
