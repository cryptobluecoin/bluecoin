...
const std::string BluecoinGUI::DEFAULT_UIPLATFORM =
...
BluecoinGUI::BluecoinGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &BluecoinGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &BluecoinGUI::showModalOverlay);
...
void BluecoinGUI::createActions()
{
    ...
    connect(overviewAction, &QAction::triggered, this, &BluecoinGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &BluecoinGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &BluecoinGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Bluecoin Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Bluecoin Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a bluecoin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Bluecoin command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &BluecoinGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &BluecoinGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &BluecoinGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &BluecoinGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &BluecoinGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &BluecoinGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &BluecoinGUI::setCurrentWallet, Qt::QueuedConnection);
                    activity->open(path);
                });
            }
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &BluecoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &CreateWalletActivity::created, this, &BluecoinGUI::setCurrentWallet);
            activity->create();
        });
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_mask_values_action, &QAction::toggled, this, &BluecoinGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &BluecoinGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &BluecoinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &BluecoinGUI::showDebugWindow);
}

void BluecoinGUI::createMenuBar()
{
#ifdef Q_OS_MACOS
...
void BluecoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &BluecoinGUI::setCurrentWalletBySelectorIndex);
...
void BluecoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &BluecoinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &BluecoinGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &BluecoinGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &BluecoinGUI::showProgress);
...
#ifdef ENABLE_WALLET
void BluecoinGUI::enableHistoryAction(bool privacy)
{
    historyAction->setEnabled(!privacy);
    if (historyAction->isChecked()) gotoOverviewPage();
}

void BluecoinGUI::setWalletController(WalletController* wallet_controller)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &BluecoinGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &BluecoinGUI::removeWallet);
...
WalletController* BluecoinGUI::getWalletController()
{
    return m_wallet_controller;
}

void BluecoinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &BluecoinGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &BluecoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &BluecoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &BluecoinGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &BluecoinGUI::incomingTransaction);
    connect(this, &BluecoinGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void BluecoinGUI::removeWallet(WalletModel* walletModel)
{
...
void BluecoinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void BluecoinGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void BluecoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void BluecoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void BluecoinGUI::createTrayIcon()
{
...
void BluecoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &BluecoinGUI::toggleHidden);
...
void BluecoinGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void BluecoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void BluecoinGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void BluecoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void BluecoinGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void BluecoinGUI::openClicked()
{
...
void BluecoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void BluecoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void BluecoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void BluecoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void BluecoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void BluecoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void BluecoinGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void BluecoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Bluecoin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void BluecoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void BluecoinGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void BluecoinGUI::updateHeadersSyncProgressLabel()
{
...
void BluecoinGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void BluecoinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &BluecoinGUI::quitRequested);
...
void BluecoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void BluecoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void BluecoinGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &BluecoinGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &BluecoinGUI::show);
                e->ignore();
            }
        }
    }
...
void BluecoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void BluecoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void BluecoinGUI::incomingTransaction(const QString& date, BluecoinUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(BluecoinUnits::formatWithUnit(unit, amount, true));
...
void BluecoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void BluecoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool BluecoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool BluecoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void BluecoinGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void BluecoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void BluecoinGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void BluecoinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void BluecoinGUI::updateWindowTitle()
{
...
void BluecoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void BluecoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void BluecoinGUI::detectShutdown()
{
...
void BluecoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void BluecoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(BluecoinGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = BluecoinGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void BluecoinGUI::subscribeToCoreSignals()
{
...
void BluecoinGUI::unsubscribeFromCoreSignals()
{
...
bool BluecoinGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<BluecoinUnit> units = BluecoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const BluecoinUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, BluecoinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const BluecoinUnit u : BluecoinUnits::availableUnits()) {
        menu->addAction(BluecoinUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(BluecoinUnit newUnits)
{
    setText(BluecoinUnits::longName(newUnits));
}
