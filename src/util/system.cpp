...
const char * const BLUECOIN_CONF_FILENAME = "bluecoin.conf";
const char * const BLUECOIN_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bluecoin-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", BLUECOIN_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Bluecoin
    // macOS: ~/Library/Application Support/Bluecoin
    // Unix-like: ~/.bluecoin
#ifdef WIN32
    // Windows
...
{
    return GetConfigFile(*this, GetPathArg("-conf", BLUECOIN_CONF_FILENAME));
}
